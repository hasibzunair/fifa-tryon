...
from models.modelsself import create_model
...
if not os.path.exists("sample"):
    os.mkdir(os.path.join("sample"))
if not os.path.exists(os.path.join("sample", "inputs")):
    os.mkdir(os.path.join("sample", "inputs"))
    os.mkdir(os.path.join("sample", "masked_inputs"))
    os.mkdir(os.path.join("sample", "preds"))
...
print(f"Training on {opt.dataroot} dataset.")
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(dataset, start=epoch_iter):        
        ...
        # masked clothing
        a = Variable(data['edge'].cuda())
        c = Variable(data['color'].cuda())
        m = Variable(data['mask'].cuda())
        masked_clothes = a * c * (1-m)
        ...
        #val = to_img(masked_clothes)
        #plt.imshow(val); plt.show()
        #import ipdb; ipdb.set_trace()
        
        
        ############## Forward Pass ######################
        losses, fake_image, L1_loss, style_loss, vgg_loss= model(Variable(data['label'].cuda()),Variable(data['edge'].cuda()),Variable(img_fore.cuda()),Variable(mask_clothes.cuda()),Variable(data['color'].cuda()),Variable(all_clothes_label.cuda()),Variable(data['image'].cuda()),Variable(data['pose'].cuda()),Variable(data['mask'].cuda()), masked_clothes)
        ...
        #loss_G = loss_dict['G_GAN']+loss_dict.get('G_GAN_Feat',0)+loss_dict.get('G_VGG',0)+torch.mean(L1_loss)
        loss_G = L1_loss #+ vgg_loss #+ style_loss 
        ...
        if step % 100 == 0:
            # preds
            cv_img=(fake_image[0].permute(1,2,0).detach().cpu().numpy()+1)/2
            rgb=(cv_img*255).astype(np.uint8)
            bgr=cv2.cvtColor(rgb,cv2.COLOR_RGB2BGR)
            cv2.imwrite('sample/preds/test'+str(step)+'.jpg',bgr)
            
            # masekd inputs
            xx=(masked_clothes[0].permute(1,2,0).detach().cpu().numpy()+1)/2
            xx=(xx*255).astype(np.uint8)
            xx=cv2.cvtColor(xx,cv2.COLOR_RGB2BGR)
            cv2.imwrite('sample/masked_inputs/test'+str(step)+'.jpg',xx)
            
            # inputs
            yy=(c[0].permute(1,2,0).detach().cpu().numpy()+1)/2
            yy=(yy*255).astype(np.uint8)
            yy=cv2.cvtColor(yy,cv2.COLOR_RGB2BGR)
            cv2.imwrite('sample/inputs/test'+str(step)+'.jpg',yy)
