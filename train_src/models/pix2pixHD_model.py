...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        ################################ Load fabricator weights ################################
        # Uncomment this line
        print("Loading Fabricator weights for Warper.")
        self.load_network(self.Unet, 'U', "latest", "checkpoints/deca_viton_fabricator")
        print("Loaded..")
        #########################################################################################
        ...
        if self.isTrain:
            ...
            ################################ Load FIFA trained on VITON weights ################################
            viton_pretrain = False
            if viton_pretrain:
                pretrained_path = "checkpoints/eccv_vton/"
                epc = "latest"
                print("Loading weights for pre-training.")
                self.load_network(self.Unet, 'U', epc, pretrained_path)
                self.load_network(self.G1, 'G1',epc, pretrained_path)
                self.load_network(self.G2, 'G2', epc, pretrained_path)
                self.load_network(self.G, 'G', epc, pretrained_path)
                self.load_network(self.D, 'D', epc, pretrained_path)
                self.load_network(self.D1, 'D1', epc, pretrained_path)
                self.load_network(self.D2, 'D2', epc, pretrained_path)
                self.load_network(self.D3, 'D3', epc, pretrained_path)
                self.load_network(self.optimizer_G, 'OG', epc, pretrained_path)
                self.load_network(self.optimizer_D, 'OD', epc, pretrained_path)
                print("Loaded weights for pre-training")
            #########################################################################################
    ...
    def forward(self,label,pre_clothes_mask,img_fore,clothes_mask,clothes,all_clothes_label,real_image,pose,mask):
        ...
        # CWM module
        # clothes -> image of target cloth
        # clothes_mask -> mask of cloth which person is wearing, NOT TARGET CLOTH MASK!
        # pre_clothes_mask -> mask of target clothing
        
        #import ipdb; ipdb.set_trace()
        
        fake_c, warped, warped_mask,rx,ry,cx,cy,rg,cg = self.Unet(clothes, clothes_mask,pre_clothes_mask, mask)
        # fake_c -> refined output from unet
        # warped -> stn output
        # warped_mask -> warped mask from stn
        
        #warped_maski = to_img(warped_mask)
        # plt.imshow(fake_ci); plt.show()
        #import ipdb; ipdb.set_trace()
        ...
        loss_G_VGG += self.criterionVGG.warp(fake_c, real_image*clothes_mask) * 20 # 20 before
        ...
        # MSSIML1 loss
        # 10 -> 0.8306 SSIM, 26.20 FID
        # 20 -> 0.8254 SSIM, 31.02 FID
        # try 5 -> 0.8297, 30.73
        # 0.2 -> 0.8242, 23.29
        # 0.02 -> 
        ...
        # With all loss terms above, and this, it works best!
        # 5 is best with SSIM, but FID is bad
        # 10 is good
        # 50 is okay
        # 20, 82.79 
        
        
        # 15, 83.61 with inp_new
        # 50 is good
        # 
        #L1_loss += self.criterionMSSSIML1(fake_c, real_image*clothes_mask) * 100
        msssim_loss = self.criterionMSSSIML1(fake_c, real_image*clothes_mask) * 15
        
        
        # 0.2 is good with L1_loss
        #L1_loss +=self.criterionDWTSSIM(fake_c, real_image*clothes_mask) * 5
        ...
        #
        #style_loss=self.criterionStyle(fake_c, real_image*clothes_mask)*200
       ...
       ,L1_loss,style_loss,fake_cl,warped,clothes,CE_loss,rx*0.1,ry*0.1,cx*0.1,cy*0.1,rg*0.1,cg*0.1, msssim_loss]
