...
def define_UnetMaskSelf(input_nc, gpu_ids=[]):
    netG = UnetMaskSelf(input_nc,output_nc=4)
    netG.cuda(gpu_ids[0])
    netG.apply(weights_init)
    return netG
...
class DWTSSIMLoss(nn.Module):
    def __init__(self):
        super(DWTSSIMLoss, self).__init__()
...
class UnetMask(nn.Module):
    ...
    def forward(self, input, refer, mask, scribble):
        # input -> image of target cloth
        # refer -> mask of cloth which person is wearing, NOT TARGET CLOTH MASK!
        # mask -> mask of target clothing
        ...
        # masked target clothing image
        #input_msk = input * (1 - scribble)
        
        
        #import ipdb; ipdb.set_trace()
        
        # masked clothing as input to stn
        #input, warped_mask,rx,ry,cx,cy,rg,cg = self.stn(input_msk, torch.cat([mask, refer, input_msk], 1), mask)
        ####################
        
        # transformed target cloth by STN and mask of person cloth together
...
# class UnetMask_Teacher(nn.Module):
#     def __init__(self, input_nc, output_nc=3):
#         super(UnetMask_Teacher, self).__init__()
#         self.stn = STNNet()
#         #ipdb.set_trace()
#         nl = nn.InstanceNorm2d
#         self.conv1 = nn.Sequential(*[nn.Conv2d(input_nc, 64, kernel_size=3, stride=1, padding=1), nl(64), nn.ReLU(),
...
#         #val = to_img(input_masked)
#         #plt.imshow(val); plt.show()
#         #import ipdb; ipdb.set_trace()
        
#         # Mask clothing as input
#         conv1 = self.conv1(torch.cat([refer.detach(), input.detach()], 1))
#         # Original clothing as input
#         #conv1 = self.conv1(torch.cat([refer.detach(), input.detach()], 1))
#         pool1 = self.pool1(conv1)
...
class UnetMaskSelf(nn.Module):
    def __init__(self, input_nc, output_nc=3):
        super(UnetMaskSelf, self).__init__()
        self.stn = STNNet()
        #ipdb.set_trace()
        nl = nn.InstanceNorm2d
        self.conv1 = nn.Sequential(*[nn.Conv2d(input_nc, 64, kernel_size=3, stride=1, padding=1), nl(64), nn.ReLU(),
    ...
    def forward(self, input, refer, mask, scribble):
        ...
        # transformed target cloth by STN and mask of person cloth together
        input_, warped_mask,rx,ry,cx,cy,rg,cg = self.stn(input, torch.cat([mask, refer, input], 1), mask)
        ...
        #masked_clothes = input * (scribble_mask)
        
        #val = to_img(warped_mask)
        #plt.imshow(val); plt.show()
        #import ipdb; ipdb.set_trace()
