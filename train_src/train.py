...
os.makedirs('sample/training_outputs',exist_ok=True)
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(dataset, start=epoch_iter):        
        ...
        loss_G = loss_dict['G_GAN']+loss_dict.get('G_GAN_Feat',0)+loss_dict.get('G_VGG',0)+torch.mean(L1_loss+CE_loss+rx+ry+cx+cy+rg+cg) + msssim_loss
        ...
        if step % 100 == 0:
            ...
            cv2.imwrite('sample/training_outputs/test'+str(step)+'.jpg',bgr)
        ...
        if epoch_iter >= dataset_size: # dataset_size
            break
